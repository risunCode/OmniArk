...
const ASSIST_SYSTEM_PROMPT = `Kamu adalah AI Prompt Engineer untuk Codex image generation.
...
const IMAGE_PROVIDER_PREFIX = ["codex-image", "codex-video"];
  ...
  try {
    if (!result.success || !result.response) {
      void recordRequest({
        ...
        requestBody: prepareLogBody({ ...request, _omniark: { source: "image-studio.assist" } }),
...
imageStudioRouter.post("/generate", async (c) => {
  ...
  const model = type === "video" ? "codex-video" : "codex-image";
  ...
  try {
    if (!result.success || !result.response) {
      void recordRequest({
        ...
        requestBody: prepareLogBody({ ...request, _omniark: { source: "image-studio.generate" } }),
        accountQuotaBefore: quotaBefore,
        accountQuotaAfter: quotaBefore,
      });
